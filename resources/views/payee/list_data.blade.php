    ...
    @foreach($payee as $v)
        <div class="payeelist col-lg-12">
            <a class="checkeds" payee_id="{{$v['id']}}" payee_name="{{$v['bank_account_name']}}">
                <div class="info"
                     style="border: 1px solid <?php echo $v['is_default'] == 1 ? '#cc2323' : '#a0a1a3'; ?>;">
                    <div @if($v['is_public'])class="text-danger"@endif>{{$v['bank_account_name']}}</div>
                    <div>{{$v['bank_other']}}</div>
                    <div>{{$v['bank_account']}}</div>
                </div>
            </a>
            &nbsp;<span class="default" urlid="{{$v['id']}}">设为默认</span>&nbsp;&nbsp;
            <a class="edit text-primary" href="{{asset(route('payee_create_or_edit',['id'=>$v['id']]))}}"
               style="">修改</a>
            <a class="delete text-danger" onclick="delPayee({{$v['id']}})">删除</a>
        </div>
